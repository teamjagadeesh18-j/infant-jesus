...
function StepRow({ step, index, progress, total }: { step: TrackStep; index: number; progress: MotionValue<number>; total: number }) {
  ...
  return (
    ...
    <div className="relative flex items-center min-h-[130px] w-full">
      <m.div style={{ scale }} className="absolute left-1/2 -translate-x-1/2 z-20 flex items-center justify-center">
      ...
      <m.div style={{ opacity, x: cardX }} className={`w-full md:w-[42%] ${isLeft ? "md:mr-auto md:pr-10 md:text-right" : "md:ml-auto md:pl-10 md:text-left"} pl-10 md:pl-0`}>
    ...
    <LazyMotion features={domAnimation}>
      ...
      <section className={`bg-white dark:bg-black py-10 px-6 relative overflow-hidden ${className || ""}`}>
        ...
        <div ref={containerRef} className="relative max-w-2xl mx-auto z-10">
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-[2px] bg-neutral-200 dark:bg-neutral-800 rounded-full" />
          <m.div style={{ height: trackLength }} className="absolute left-1/2 -translate-x-1/2 top-0 w-[2px] bg-gradient-to-b from-orange-400 via-blue-400 to-purple-400 rounded-full origin-top z-10" />
          <m.div style={{ top: dotTop }} className="absolute left-1/2 -translate-x-1/2 z-30 -mt-1.5">
